...
import { useMemo, useImperativeHandle, forwardRef, useEffect, useState, useRef, useCallback } from 'react';
...
type RFBConstructor = new (target: HTMLElement, url: string, options?: object) => ExtendedRFB;

let cachedRFB: RFBConstructor | null = null;

async function loadRFB(): Promise<RFBConstructor> {
  if (cachedRFB) return cachedRFB;
  const mod = await eval('import("@novnc/novnc/lib/rfb")');
  cachedRFB = mod.default;
  return cachedRFB as RFBConstructor;
}
...
export const VNCClient = forwardRef<ComputerDisplayRef, ComputerDisplayProps>(({
  ...
  useEffect(() => {
    loadRFB().then(setRFBClass).catch(err => onError?.(err.message));
  }, [onError]);
  ...
  const connect = useCallback(() => {
    if (!RFBClass || !vncRef.current || !url || !isMounted.current || connecting.current || isDisconnecting.current) return;

    connecting.current = true;

    setTimeout(() => {
      if (!isMounted.current || !vncRef.current || !RFBClass) {
        connecting.current = false;
        return;
      }

      try {
        const rfb = new RFBClass(vncRef.current, url, {
          credentials: {
  ...
  }, [RFBClass, url, credentials, background, readOnly, scaleViewport, clipViewport, resizeSession, showDotCursor, compressionLevel, qualityLevel, onConnect, onDisconnect, onError, onClipboard, disconnect]);
  ...
  useEffect(() => {
    if (RFBClass && url) {
      disconnect();
      const timer = setTimeout(() => isMounted.current && connect(), 100);
      return () => clearTimeout(timer);
    }
  }, [RFBClass, url, connect, disconnect]);
